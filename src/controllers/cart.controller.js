...
const addProduct = async (req,res) => {
    ...
    if(!data) {
        ...
        await Cart.updateOne({_id : id},{$push : {products : {ProdId : ProdId}}})
...
const getCartProduct = async (req,res) => {
    ...
    if (!data){
        ...
        for(let i = 0; i<data.products.length ; i++ ){
            for (let j = 0 ; j < branch.products.length; j++){
                if (data.products[i].ProdId == branch.products[j]._id){
                    products.push(branch.products[j])
                }
            }
        }
